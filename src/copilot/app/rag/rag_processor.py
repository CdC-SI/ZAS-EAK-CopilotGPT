...
from .retrieval.client import RetrieverClient
...
class RAGProcessor:
    ...
    def retrieve(self, db: Session, query: str, language: str = None, tag: str = None):
        """
        ...
        """
        rows = self.retriever_client.get_documents(db, query, language=language, tag=tag)
    ...
    def process(self, db: Session, query: str, language: str = None, tag: str = None):
        """
        ...
        """
        documents = self.retrieve(db, query, language=language, tag=tag)
        logger.info(f"ID={[doc.id for doc in documents]}")
        context_docs = "\n\n".join([doc.text for doc in documents])  # TO UPDATE
        source_url = documents[0].url  # TO UPDATE

        messages = self.create_rag_message(context_docs, query)
