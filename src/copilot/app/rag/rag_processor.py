...
class RAGProcessor:
    ...
    @observe()
    def retrieve(self, db: Session, request: RAGRequest, language: str = None, tag: str = None, k: int = 0, retriever_client: RetrieverClient = None):
        """
        ...
        rows = retriever_client.get_documents(db, request.query, language=language, tag=tag, k=k)

        return rows if len(rows) > 0 else [{"text": "", "url": ""}]

    @observe()
    def process(self, db: Session, request: RAGRequest, language: str = None, tag: str = None, streaming_handler: StreamingHandler = None, llm_client: BaseLLM = None, retriever_client: RetrieverClient = None):
        """
        ...
        """
        documents = self.retrieve(db, request, language=language, tag=tag, k=self.k_retrieve, retriever_client=retriever_client)
        ...
        return streaming_handler.generate_stream(event_stream, source_url)

    def process_request(self, db: Session, request: RAGRequest, language: str = None, tag: str = None):
