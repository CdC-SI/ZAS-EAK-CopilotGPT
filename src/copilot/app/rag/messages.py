...
class MessageBuilder:
    ...
    @observe()
    def build_chat_prompt(self, context_docs: List[Dict], query: str, conversational_memory: str) -> Union[List[Dict], str]:
        """
        ...
        if self.llm_model in SUPPORTED_OPENAI_LLM_MODELS + SUPPORTED_AZUREOPENAI_LLM_MODELS + SUPPORTED_GROQ_LLM_MODELS:
            if self.language == "de":
                ...
                prompt = RAG_SYSTEM_PROMPT_FR.format(context_docs=context_docs, query=query, conversational_memory=conversational_memory)
                return [{"role": "system", "content": prompt},]
            elif self.language == "it":
                prompt = RAG_SYSTEM_PROMPT_IT.format(context_docs=context_docs, query=query, conversational_memory=conversational_memory)
                return [{"role": "system", "content": prompt},]
            else:
                prompt = RAG_SYSTEM_PROMPT_DE.format(context_docs=context_docs, query=query, conversational_memory=conversational_memory)
                return [{"role": "system", "content": prompt},]
